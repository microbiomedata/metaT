task bbmap_mapping{
	...
	command {
		#hisat2 --dta -p ${no_of_cpus} --un-conc unaligned.fastq -x ${hisat_db_name} -1 ${rna_clean_reads[0]} -2 ${rna_clean_reads[1]} | samtools sort | samtools view -Su -o mapped_sorted.bam
                bbmap.sh nodisk=true interleaved=false ambiguous=all in1=${rna_clean_reads[0]} in2=${rna_clean_reads[1]} ref=${hisat2_ref_dbs} out=mapped.bam covstats=covstats.txt bamscript=to_bam.sh nhtag=t
