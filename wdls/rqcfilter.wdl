...
task rqcfilter {
     ...
     String container
     String database
     String rqcfilterdb = "/refdata/RQCFilterData"
     ...
     command<<<
        ...
        set -eo pipefail
        rqcfilter2.sh -Xmx${default="60G" memory} threads=${jvm_threads} ${chastityfilter} jni=t in=${input_file} path=filtered rna=t trimfragadapter=t qtrim=r trimq=0 maxns=3 maq=3 minlen=51 mlf=0.33 phix=t removehuman=t removedog=t removecat=t removemouse=t khist=t removemicrobes=t sketch kapa=t clumpify=t tmpdir= barcodefilter=f trimpolyg=5 usejni=f rqcfilterdata=${rqcfilterdb}  > >(tee -a ${filename_outlog}) 2> >(tee -a ${filename_errlog} >&2)
